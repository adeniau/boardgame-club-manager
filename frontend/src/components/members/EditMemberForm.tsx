...
const EditMemberForm: React.FC = () => {
  ...
  const loadMember = async (memberId: number) => {
    try {
      ...
      memberForm.reset(memberData);
      
      // Initialiser l'image si elle existe
      if (memberData.picture) {
        imageUpload.setPreview(MembersService.getImageUrl(memberData.picture));
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!member || !memberForm.validateForm()) {
    ...
    try {
      ...
      const memberData = memberForm.getUpdateRequest();
      ...
      setSuccess(VALIDATION_RULES.SUCCESS.MEMBER_UPDATED);
      
      // Rediriger vers la page du membre après un délai
      setTimeout(() => {
        navigate(`/members/${updatedMember.id}`);
      }, 1500);
    ...
    return (
      <div className="max-w-2xl mx-auto">
        <div className="bg-green-50 border border-green-200 rounded-lg p-6 text-center">
          ...
          <h3 className="text-lg font-medium text-green-800 mb-2">Membre modifié avec succès !</h3>
      ...
      <div className="bg-white shadow rounded-lg">
        <div className="px-6 py-4 border-b border-gray-200">
          <h1 className="text-xl font-semibold text-gray-900">
            Modifier {member.firstname} {member.name}
          </h1>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-6 space-y-6">
          ...
          <ImageUpload
            ...
            onRemove={() => imageUpload.handleRemoveImage(MembersService.getImageUrl(member.picture))}
            onError={setError}
            disabled={memberForm.isSubmitting}
            ...
            {memberForm.formData.admin === '1' && (
              <div>
                <label htmlFor="admin_password" className="block text-sm font-medium text-gray-700">
                  Nouveau mot de passe administrateur (optionnel)
                </label>
                <input
                  type="password"
                  id="admin_password"
          ...
          <div className="flex justify-end space-x-3 pt-6 border-t border-gray-200">
            ...
            <button
              type="submit"
              disabled={memberForm.isSubmitting || !memberForm.hasChanges(member)}
